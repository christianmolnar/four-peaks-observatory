...
async function updateMetadata() {
  ...
  for (const image of allImages) {
    ...
    if (isNewEntry) {
      ...
      // For all image types, only update fields that are missing or empty
      const entry = existingMetadata[image.filename];
      let needsDateUpdate = false;

      // Only update missing fields, never overwrite existing values
  ...
  if (newEntries > 0 || updatedEntries > 0 || deletedEntries > 0) {
    ...
    console.log(`✅ Updated metadata file (merged, admin edits preserved)`);
  } else {

...
export async function GET(request: Request) {
  try {
    ...
    const customClearSkyUrl = searchParams.get('customClearSkyUrl');
    // Always use the unified parser (removed enhanced parser complexity)
    
    // Parse factor weights from query parameters
    const factorWeights: Record<string, number> = {};
    for (const [key, value] of searchParams.entries()) {
      if (key.startsWith('factorWeight_')) {
        const factorName = key.replace('factorWeight_', '');
        factorWeights[factorName] = parseFloat(value) || 0;
      }
    }
    
    // DEBUG: Log the parsed factor weights
    console.log('[API] Parsed factor weights from query params:', factorWeights);
    ...
    const clearSkyChartUrl = customClearSkyUrl || criteria.location.clearSkyChartUrl;
    
    const today = new Date();
    
    // Use unified Clear Sky parser with RGB color scale
    console.log('[API] Using unified Clear Sky parser with user-specified RGB color scale');
    ...
    return NextResponse.json({
      ...
      location: chartData.location || criteria.location.name,
      observingWindow: {
        start: formatTime(observingWindow.start),
        end: formatTime(observingWindow.end),
        totalHours: observingWindow.totalHours
